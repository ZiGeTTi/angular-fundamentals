...
export class EventService {
  ...
  getEvents(): Observable<IEvent[]> {
    ...
    setTimeout(() => { subject.next(EVENTS); subject.complete(); }, 100) //observable üzerinden dönecez datayı, asenkron gösternek için
    return subject
  }

  getEvent(id: number): IEvent {
    const record = EVENTS.find(event => event.id === id)!
...
const EVENTS: IEvent[] = [
  {
    ...
    onlineUrl: '/assets/images/angularconnect-shield.png',
    sessions: [
      {
        id: 1,
